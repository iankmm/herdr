...
対象ペインで認識済みのエージェントが動作している場合、分割先では同じ種類のエージェント実行ファイルを新しく起動し、対象ペインの現在の作業ディレクトリを使用します。明示した `--cwd` は引き続き優先されます。通常のシェルまたは未認識のエージェントを含むペインを分割した場合は、従来どおりシェルペインを作成します。
