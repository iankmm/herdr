...
元のペインに認識済みのエージェントが含まれていても通常のシェルを強制するには、`--shell` を追加します。
...
`pane.view_file` は `target_pane_id`、`path`、および省略可能な 1 始まりの `line` と `column` を受け取ります。対象ペインからパスを解決し、右側に管理対象の読み取り専用 Vim 分割を開きます。その対象ペインに以前の管理対象ビューアーがある場合は置き換えます。新しいビューアーは `.result.pane` で返されます。
