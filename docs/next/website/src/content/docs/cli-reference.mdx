...
herdr pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--shell] [--focus] [--no-focus]
...
When the target pane is running a recognized agent, the split starts a fresh
instance of the same agent executable and uses the target pane's live working
directory. An explicit `--cwd` still takes precedence. Splitting a plain shell
or a pane with an unknown agent keeps the normal shell-pane behavior.
Pass `--shell` to force a plain shell split when the target contains an agent.
