...
| Action | Key |
| --- | --- |
| Open a plain terminal below at 30% height | `prefix+t` |
...
Hold `ctrl` and click a visible file reference in a terminal pane to open it in a read-only Vim pane on the right. Herdr recognizes paths such as `README.md:44-50`, `src/app.rs:72:5`, and `./infra/page.tsx`, resolves them from the source pane's live working directory and workspace checkout, and moves the cursor to the referenced line and column.

Each source pane owns one managed file viewer. Clicking another file from that same pane replaces the existing viewer in place, preserving its split geometry; different source panes keep independent viewers. Vim starts with `-R -M -n`, so the viewer is intended for reading rather than editing. This is a UI safeguard, not an operating-system sandbox. Use `:q` to close the viewer.
