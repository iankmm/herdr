...
Add `--shell` to force a plain shell when the source pane contains a recognized
agent.
...
`pane.view_file` accepts `target_pane_id`, `path`, and optional one-based `line` and `column` values. It resolves the path from the target pane, opens a managed read-only Vim split on the right, and replaces the target pane's previous managed viewer if one exists. It returns the new viewer as `.result.pane`.
