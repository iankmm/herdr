...
即使源窗格包含已识别的智能体,也可以添加 `--shell` 强制创建普通 shell。
...
`pane.view_file` 接受 `target_pane_id`、`path`,以及可选的从 1 开始的 `line` 和 `column`。它从目标窗格解析路径,在右侧打开一个受管理的只读 Vim 分割;如果该目标窗格已有受管理的查看器,则原位替换。新查看器通过 `.result.pane` 返回。
