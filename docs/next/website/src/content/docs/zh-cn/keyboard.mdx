...
按住 `ctrl` 并点击终端窗格中可见的文件引用,即可在右侧的只读 Vim 窗格中打开它。Herdr 能识别 `README.md:44-50`、`src/app.rs:72:5` 和 `./infra/page.tsx` 等路径,从来源窗格的实时工作目录和工作区检出目录解析文件,并把光标移动到引用的行和列。

每个来源窗格拥有一个受管理的文件查看器。从同一窗格点击另一个文件时,现有查看器会在原位置平滑替换,并保留分割布局;不同来源窗格的查看器彼此独立。Vim 使用 `-R -M -n` 启动,因此该窗格用于阅读而非编辑。这是界面层面的保护,不是操作系统沙箱。使用 `:q` 关闭查看器。
